...
//     const userPromise = dohvatiSvePromise();
...
function initIndex() {
    const foodPromise = dohvatiSvePromise();

    foodPromise
    .then(data => {
        let noviData = data.slice(0, 6);
...
function prikaziPraznuNarudzbinu() {
    $("#controls").hide();
    $("#contact-form").hide();
    let ispis = `<h4 class="mt-5">Your cart is empty, go to <a href="menu.html">menu</a> page</h4>`;
